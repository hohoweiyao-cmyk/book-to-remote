...
### 为什么不改用 Telegram bot 通道（2026-09-19 评估，勿重做）

Z-Library 支持绑定「个人电报机器人」（BotFather 建 bot → 把 token 粘回 z-lib 个人资料页），
社区有人因此认为它比网页路径更快。**实测数据与机制分析都不支持这个结论。**
...
|---|---|
| 搜索（20 条结果） | 16.3s |
| 下载一本 206KB epub | 17.8s |
| 本地浏览器驱动 | 0.3s（实例常驻，非每次成本） |
| **合计** | **34.1s** |

206KB 走任何正常链路都是毫秒级 —— 这 17.8s 几乎全在 **Z-Library 服务端的生成与限流**。

**三条推翻理由**：

1. **瓶颈不在通道**。33.8s / 34.1s（99%）消耗在 Z-Library 服务端，TG 拿书同样要过这一段；
   换通道能省下的只有那 0.3s。
2. **无法自动落盘**。Bot API 的 `getUpdates` 只返回「用户发给 bot」的更新，**读不到 bot 自己发出的消息**，
   因此拿不到 `file_id`，也就无法用 `getFile` 取文件。要自动化必须另建 **MTProto userbot**
   （api_id / api_hash + 手机号验证码登录 + 持久化 session 文件）—— 这是全新的高敏凭证体系，
   与「零授权」目标直接冲突。
3. **只能覆盖四段链路的第 1 段**。整条链路是「找书下载 → 本机落盘 → 微信读书 → Kindle」，
   TG 只管第 1 段，后三段仍要回到现有实现。它是前置入口，不是替代方案。

**另一事实**：TG 与网页共用同一 Z-Library 账号额度，换通道不增加每日可下载量。

**TG 路径真正合适的场景**：**手机端**、没有电脑时随手找书 —— 那是它的主场，与本链路并行不冲突。
（本机现状：未安装 Telegram 客户端，`api.telegram.org` HTTP 层直连可达。）

**社区已有现成实现（2026-09-19 已核源码）**：`github.com/longhaiqwe/telegram-book-download`，
作者龙海（X @longhaiqwe123），2026-09-17 发布。要用 TG 路径就优先用它，不要自己重写。

| 项 | 核实结果 |
|---|---|
| 机制 | Telethon 以**个人 TG 账号**操作 Z-Library 私人 bot（正是上文推演的 MTProto userbot 路线） |
| 覆盖范围 | 搜索 → 选版本 → 下载 → 规范命名。README 明确写「**不负责运营机器人或默认导入微信读书**」 |
| 后续环节 | 交给作者的另一个 skill `book-bridge`（即：本任务的后三段仍需另外的组件） |
| 代码质量 | 高。单文件 9.5KB：`os.umask(0o077)`、配置 600 / session 目录 700、按钮文本二次校验、FloodWait 专门处理、`.source.json` 记 SHA-256 与消息 ID、HTML 误下检测、`.unverified` 标记、超时 180s / 下载 900s |
| 前置成本 | api_id + api_hash（作者自述申请屡报 ERROR，最终花钱请第三方解决）+ 手机验证码 + 两步验证 + 可能需 SOCKS5 |

**一处修正**：本机实测 Telethon 直连 Telegram 的 MTProto 握手 **3.08s 成功，不需要代理**
（TCP `149.154.167.51:443` 可连）。但「TG 下载文件是否快于网页下载」**本机无法验证**（需授权账号）——
上文「换通道省不掉」的结论只对「搜索 16.3s + 服务端生成 17.5s」两段成立，
**传输段存疑**：TG 下载走 TG 的 CDN，网页下载走 Z-Library 的限速通道，两者未必等速。
禁止在没有实测的情况下断言哪边快。
